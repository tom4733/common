#!/bin/bash
# https://github.com/tom4733/build-actions
# common Module by tom4733
...
if [[ -n "${BENDI_VERSION}" ]]; then
  ...
  sudo rm -rf build/common && git clone -b main --depth 1 https://github.com/tom4733/common build/common
...
git clone -b main --depth 1 https://github.com/tom4733/build-actions shangyou
...
if [[ "${GIT_REPOSITORY}" == "tom4733/build-actions" ]]; then
  rm -rf backups
  BANBEN_SHUOMING="Update $(date +%Y.%m%d.%H%M.%S)"
fi
if [[ "${GIT_REPOSITORY}" == "tom4733/autobuild" ]]; then
  rm -rf backups
fi
git add .
...
curl -fsSL https://raw.githubusercontent.com/tom4733/common/main/common.sh -o common.sh
if [[ $? -ne 0 ]]; then
  wget -q https://raw.githubusercontent.com/tom4733/common/main/common.sh -O common.sh
fi
...
if [[ -n "${BENDI_VERSION}" ]]; then
  ...
  export GIT_REPOSITORY="tom4733/build-actions"
  sudo rm -rf build common.sh
